# This script produces Table D.1 in the Supplemental Appendix of the empirical application (monthly data)
...
rolling <- function(excess,weight) {
  ...
  # perest <- list(c(mur,sigma2,sharpe))
  # 
  # return(perest)
  # }
  ...
  # retra <- matrix(c(NA), R, ncol(excess)) #R*p matrix
  # for (k in 1:R) {
  #   retra[k,1:length(rets[k,StockInd[k,]])] = 1+as.matrix(t(rets[k,StockInd[k,]]))
  #   
  #   for (i in 1:length(retra[k,])) {
  #     retra[k,i]=retra[k,i]/(1+portret[k])
  #   }
  # }
...
######################################################################################
#####Monthly Data####
FF5_MONTHLY <- as.matrix(FF5_MONTHLY)
FF1 <- FF5_MONTHLY[,1]
FF1 <- as.matrix(FF1)
FF3 <- FF5_MONTHLY[,1:3]
FF5 <- FF5_MONTHLY[,1:5]
rf <- FF5_MONTHLY[,6]
adj_returns2 <- as.matrix(monthly_excess)
...
for(j in 1:R)  {
  ...
  for (i in 1:ncol(adj_returns2)){
    if (!is.na(set1[j,i]) & !is.na(set1[(j+180),i]) ){ #240 for R=240
      list1 = c(list1,i)
    }
  }
  StockInd[j,1:length(list1)] = list1
  set1 = set1[,list1]
  set1 = set1[j:(j+179),] #239 for R=240
  set11 <- zoo(set1)
  set12 <- na.locf(set11, fromLast = FALSE)
  ...
  ########NLW#############
  NLWTheta = solve(covU_NL) -  solve(covU_NL)%*%t(betas)%*%solve( solve(cov(covariate))+
                                                                    betas%*% solve(covU_NL)%*%t(betas))%*%betas%*% solve(covU_NL)
  ...
  #####FINAL WEIGHT COMPLETIONS#####
  
  weightCLIME_global[j,1:length(list1)]  <- t(wCLIME[[1]])
  weightCLIME_mkw[j,1:length(list1)]  <- t(wCLIME[[2]])
  weightCLIME_fan[j,1:length(list1)]  <- t(wCLIME[[3]])
  
  weightLW_global[j,1:length(list1)]  <- t(wLW[[1]])
  weightLW_mkw[j,1:length(list1)]  <- t(wLW[[2]])
  weightLW_fan[j,1:length(list1)]  <- t(wLW[[3]])
  
  
  weightPOET_global[j,1:length(list1)]  <- t(wPOET[[1]])
  weightPOET_mkw[j,1:length(list1)]  <- t(wPOET[[2]])
  weightPOET_fan[j,1:length(list1)]  <- t(wPOET[[3]])
  
  weightNLW_global[j,1:length(list1)]  <- t(wNLW[[1]])
  weightNLW_mkw[j,1:length(list1)]  <- t(wNLW[[2]])
  weightNLW_fan[j,1:length(list1)]  <- t(wNLW[[3]])
  
  weightPOETproj_global[j,1:length(list1)]  <- t(wPOET_proj[[1]])
  weightPOETproj_mkw[j,1:length(list1)]  <- t(wPOET_proj[[2]])
  weightPOETproj_fan[j,1:length(list1)]  <- t(wPOET_proj[[3]])
  
  
  # 
  weightFGL_global[j,1:length(list1)]  <- t(wFGL[[1]])
  weightFGL_mkw[j,1:length(list1)]  <- t(wFGL[[2]])
  weightFGL_fan[j,1:length(list1)]  <- t(wFGL[[3]])
  # 
  weightFF1_global[j,1:length(list1)]  <- t(wFGL1[[1]])
  weightFF1_mkw[j,1:length(list1)]  <- t(wFGL1[[2]])
  weightFF1_fan[j,1:length(list1)]  <- t(wFGL1[[3]])
  
  weightFF3_global[j,1:length(list1)]  <- t(wFGL2[[1]])
  weightFF3_mkw[j,1:length(list1)]  <- t(wFGL2[[2]])
  weightFF3_fan[j,1:length(list1)]  <- t(wFGL2[[3]])
  
  weightFF5_global[j,1:length(list1)]  <- t(wFGL3[[1]])
  weightFF5_mkw[j,1:length(list1)]  <- t(wFGL3[[2]])
  weightFF5_fan[j,1:length(list1)]  <- t(wFGL3[[3]])
  
  ###NO SPARSITY IN FACTORS
  weightFF1NS_global[j,1:length(list1)]  <- t(w1[[1]])
  weightFF1NS_mkw[j,1:length(list1)]  <- t(w1[[2]])
  weightFF1NS_fan[j,1:length(list1)]  <- t(w1[[3]])
  
  weightFF3NS_global[j,1:length(list1)]  <- t(w2[[1]])
  weightFF3NS_mkw[j,1:length(list1)]  <- t(w2[[2]])
  weightFF3NS_fan[j,1:length(list1)]  <- t(w2[[3]])
  
  weightFF5NS_global[j,1:length(list1)]  <- t(w3[[1]])
  weightFF5NS_mkw[j,1:length(list1)]  <- t(w3[[2]])
  weightFF5NS_fan[j,1:length(list1)]  <- t(w3[[3]])
}
########################################
...
#####INDEX#####
adj_composite_EXCESS <- as.matrix(spx_excess_monthly)
snp_test <- adj_composite_EXCESS[(pred+1):nrow(excess),]
...
############################################
A1 = matrix(NA,12,4)
list1 = list(portfFGL_global,portfLW_global,portfPOET_global,portfCLIME_global,portfNLW_global,portfPOETproj_global, portfFF1_global, portfFF3_global, portfFF5_global, portfFF1NS_global, portfFF3NS_global, portfFF5NS_global)
...
A2 = matrix(NA,12,4)
list1 = list(portfFGL_mkw,portfLW_mkw,portfPOET_mkw,portfCLIME_mkw,portfNLW_mkw,portfPOETproj_mkw, portfFF1_mkw, portfFF3_mkw, portfFF5_mkw, portfFF1NS_mkw, portfFF3NS_mkw, portfFF5NS_mkw)
for(i in seq_along(list1)){
...
A3 = matrix(NA,11,4)
list1 = list(portfFGL_fan,portfLW_fan,portfCLIME_fan,portfNLW_fan,portfPOETproj_fan, portfFF1_fan, portfFF3_fan, portfFF5_fan, portfFF1NS_fan, portfFF3NS_fan, portfFF5NS_fan)
for(i in seq_along(list1)){
